...
object Trees {
  ...
  case class IntArrayType() extends TypeTree { override def getType: Type = TIntArray}
  case class IntType() extends TypeTree { override def getType: Type = TInt}
  case class BooleanType() extends TypeTree { override def getType: Type = TBoolean}
  case class StringType() extends TypeTree { override def getType: Type = TString}
  ...
  case class Identifier(value: String, var genericType: Option[TypeTree]) extends TypeTree
  with ExprTree with Symbolic[Symbol] {
    // The type of the identifier depends on the type of the symbol
    override def getType: Type = getSymbol match {
      case cs: ClassSymbol =>
        TObject(cs, genericType)
